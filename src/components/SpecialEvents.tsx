...
export default function SpecialEvents() {
  const [states, setStates] = useState<
    Record<string, { active: boolean; highlighted: boolean; activeSubName?: string }>
  >({});
        ...
        return (
          ...
          >
            <div className="sp-name">
              <span className="sp-icon">{EVENT_ICONS[ev.id] || "📌"}</span>
              <span className="sp-name-text">{ev.name}</span>
              {state.highlighted && <span className="sp-badge">АКТИВНО</span>}
            </div>
            <div className="sp-desc">{ev.description}</div>
            {state.activeSubName && (
              <div className="sp-sub">Сегодня: {state.activeSubName}</div>
            )}
            {ev.days && (
            ...
            {ev.subEvents && (
              <div className="sp-days">
                {ev.subEvents
                  .map(
                    (s) =>
                      `${s.name}: ${s.days.map((d) => DAY_NAMES[d]).join(", ")}`
                  )
                  .join(" | ")}
              </div>
            )}
